...
2. ~~**α-sweep 下游实验**~~ ✅ 完成,见 [[alpha-sweep-csi800-m6]]。结论:没看到 model collapse,M6 合成数据约等于真实数据作为下游训练素材
3. **Leverage 方向性** — 见 [[#open]] 和 [[a-share-positive-leverage]],sign-aware conditioning(M8)证明**不够**;候选:explicit leverage aux loss,asymmetric noise schedule,或两阶段 GARCH+InterDiff
4. **Factor-model 分析** — M6 的 576 个同 sector 窗口样本足够做经验 factor-loading 分析,验证 denoiser 确实在用 sector 信号而不是忽略
5. **更强 baseline predictor** — 当前 α-sweep IC 在噪声带,用 LightGBM + 丰富特征重跑,可能揭示 α 效应
