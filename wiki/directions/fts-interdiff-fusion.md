...
### 2026-04-20 — 重要负面发现: M16 下游**劣于** M6

见 [[lgbm-alpha-sweep-m16-vs-m6]]。10-seed LGBM α-sweep 公平对比:

| α | M6 IC (10 seeds) | M16 IC (10 seeds) |
|---|---|---|
| 0.00 (real only) | +0.0021 | +0.0021 |
| 0.50 (最优点) | **+0.0087** (t=1.68) | **+0.0033** (t=0.36) |

M16 在 stylized facts 上全面好过 M6(leverage -0.0037 vs -0.007, skew +0.118 vs -0.099, max_eig perfect),**但 α=0.5 处下游 IC 低 0.005**(paired t=-1.55 against M6)。

**解读: hinge aux loss 的 regularization 以生成多样性为代价**。为了满足 leverage ≥ target 的批级统计约束,模型压缩了 x0_pred 输出流形,idiosyncratic 变异被平滑,LGBM 没有足够的"extra variation"可学。

**教训**: Stylized-fact verdict 7/7 全绿不等于下游 utility 更好。方法论上需要补一个多样性 / 覆盖度指标(或直接跑下游 IC sweep)。

**生产决策: 切回 M6 作为下游数据增强默认模型**。M16 保留给需要 surface quality 的应用(风险模型 / 压力测试 / 论文 figure)。
